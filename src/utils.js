    ...
    Simulator = {
        ...
        Filler: {
            ...
            addBenefitsFromSelectedVillage: function(e) {
                if (e == Simulator.SIDE_ATTACKER) var t = $("#att_fill_select");
                else t = $("#def_fill_select");
                var i = $(t).val();
                    ...
                    return this.inputs.forEach(function(t, i) {
                        if ("hidden" !== t.type) {
                            if (e += '<div class="attribute-input">' + t.label + ' <select name="inputs[]">', "int" === t.type)
                                ...
                                } else "enum" === t.type && t.options.forEach(function(t, i) {
                                    e += '<option value="' + t.value + '">' + escapeHtml(t.description) + "</option>"
                                });
                            e += "</select></div>"
                        } else e += '<input type="hidden" name="inputs[]" value="' + t.value + '">'
                    }), e
                }
            }
        },
        EffectTemplates: {
            ...
            removeTemplate: function(e) {
                if (e == Simulator.SIDE_ATTACKER) var t = $("#attack_effect_templates");
                else t = $("#defense_effect_templates");
                var i = $(t).val();
                    ...
                    switch (parseInt(i.status)) {
                        case 1:
                            if (UI.SuccessMessage(i.message), Simulator.EffectTemplates.addTemplate(i.template), 0 == $('.template-list>option[value="' + i.template.id + '"]').length) {
                                if (t == Simulator.SIDE_ATTACKER) var a = $("#attack_effect_templates");
                                else a = $("#defense_effect_templates");
